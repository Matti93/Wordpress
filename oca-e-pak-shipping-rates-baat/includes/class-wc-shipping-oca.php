...
class WC_Shipping_Oca extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_title         = __( 'OCA Express Pak', 'woocommerce-shipping-oca' );
 		$this->method_description   = __( 'Obtener costos de envío de manera dinámica utilizando la API de OCA. (ES NECESARIO TENER UNA CUENTA CON OCA)', 'woocommerce' );
	...
	*/
	...
	public function calculate_shipping( $package = array() ) {
		...
		$origen_datos[] = array (
			...
			'oca_lenth' => $oca_lenth,
			'oca_width' => $oca_width,
			'oca_height' => $oca_height,
			'oca_amount' => $oca_amount,
			'oca_weightb' => $oca_weightb,	
		);
		$origen_datos = json_encode($origen_datos);
		$_SESSION['origen_datos'] = $origen_datos;
		...
		foreach($this->services as $services) {
			...
			if($services['enabled'] == 1){
				...
				$porcentaje = $oca_response->results[0]->Total * $ajuste / 100;
