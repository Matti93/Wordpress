<?php
	if (isset($_COOKIE['oca_notice'])) {
			$_SESSION['oca_notice'] = $_COOKIE['oca_notice'];
			add_action( 'admin_notices', 'oca_admin_notice' );
	}

	add_action('wp_ajax_check_sucursales', 'check_sucursales', 10);
	add_action('wp_ajax_nopriv_check_sucursales', 'check_sucursales', 10);

	add_action('wp_ajax_check_admision', 'check_admision', 10);
	add_action('wp_ajax_nopriv_check_admision', 'check_admision', 10);
	...
	function check_sucursales() {
        ...
        $oca_response = json_decode($oca_response);
				...
				$_SESSION['listado_oca'] = $listado_oca;
				$_SESSION['params_oca'] = $params;
  ...
  /**
	 ...
	 */
	add_action( 'woocommerce_checkout_update_order_meta', 'order_sucursal_main_update_order_meta_oca' );
	function order_sucursal_main_update_order_meta_oca( $order_id ) {
		session_start();
 	    if ( ! empty( $_POST['pv_centro_oca_estandar'] ) ) {
				foreach($_SESSION['listado_oca'] as $opciones){
					if($_POST['pv_centro_oca_estandar'] == $opciones->IdCentroImposicion){
						$opciones = json_encode($opciones);
						update_post_meta( $order_id, '_sucursal_oca_c', $opciones );
					}				
 				}
	    }
			$chosen_shipping = json_encode($_SESSION['chosen_shipping'] );
			$params_oca = json_encode($_SESSION['params_oca'] );
			update_post_meta( $order_id, '_params_oca', $params_oca );
	...
	function woocommerce_oca_box_create_box_content() {
		...
		if (empty($oca_shipping_label_tracking)){ ?>
			...
			jQuery('body').on('click', '#generar-oca',function(e){ 
				e.preventDefault();
 				jQuery(".oca-single-label").fadeIn(400);
				jQuery(".oca-single-label").html(' SOLO EN VERSION PREMIUM ');
			});	
			</script>
		<?php } 
	}
	...
	function oca_admin_notice() {
			...
			<div class="notice error my-acf-notice is-dismissible" >
					<p><?php print_r($_SESSION['oca_notice'] ); ?></p>
			</div>
